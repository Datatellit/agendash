...
module.exports = (agendash) => {
  ...
  app.post('/api/jobs/create', function (req, res, next) {
    agendash.createJob(req.body.jobName, req.body.jobSchedule, req.body.jobRepeatEvery, req.body.jobData, function (err, job) {
      if (err) return res.status(404).json(err)
      return res.json({created: true,data:job})
    })
  })
